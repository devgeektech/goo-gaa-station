...
export const ROUTES = [
  ...
  { method: 'GET', path: '/api/v1/admin/banners', auth: true, description: 'List banners (admin), sorted by position' },
  { method: 'POST', path: '/api/v1/admin/banners', auth: true, description: 'Create banner (admin; multipart: image, heading, text, buttonText, buttonUrl, position, isActive)' },
  { method: 'PATCH', path: '/api/v1/admin/banners/:id', auth: true, description: 'Update banner (admin; multipart supported; validates unique position)' },
  { method: 'PATCH', path: '/api/v1/admin/banners/:id/toggle', auth: true, description: 'Toggle banner active status (admin)' },
  { method: 'DELETE', path: '/api/v1/admin/banners/:id', auth: true, description: 'Delete banner (admin)' },
  ...
  { method: 'POST', path: '/api/v1/app/orders', auth: true, description: 'Place order (customer); cart-based with deliveryAddressId preferred; 30s customer cancel window before vendor is notified; returns items, totals, OTP, displayOrderId, customerCancelDeadline, cancelRemainingSeconds' },
  ...
  { method: 'POST', path: '/api/v1/app/orders/:id/cancel', auth: true, description: 'Cancel order (customer); only within 30s of placement while pending and vendor not yet notified; no vendor alert; WifiPay refund hook commented in handler until online payment is live' },
