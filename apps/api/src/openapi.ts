...
function getQueryParametersForRoute(opKey: string): Record<string, unknown>[] {
  ...
  const map: Record<string, Record<string, unknown>[]> = {
    ...
    'GET /api/v1/app/vendors': [
      ...
      query('customerLng', { type: 'number', example: 76.7297 }, false, 'Customer longitude; alias pair lat + lng also accepted'),
      query('lat', { type: 'number' }, false, 'Alias for customerLat when lng is also sent'),
      query('lng', { type: 'number' }, false, 'Alias for customerLng when lat is also sent'),
      ...discoveryFilterParams,
      ...discoveryPaginationParams,
    ],
